...
Overall, the systems assessed appeared to support the majority of data requirements. Many of the data requirements could be met with minimal additional processing as there were closely aligned data fields observed. In other cases, data could be derived from related data fields through additional, more significant processing of data for export. It is noted, however, that support may already exist with capability not assessable in a UI. There was only a single data point in one system that did not appear to be supported, with no related data field observed.
